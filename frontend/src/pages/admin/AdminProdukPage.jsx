// frontend/src/pages/admin/AdminProdukPage.jsx
import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import { adminApi } from "../../api";
import { formatRupiah, mediaUrl, onImgError } from "../../utils";

export default function AdminProdukPage() {
  const [produk, setProduk] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  useEffect(() => {
    fetchProduk();
  }, []);

  const fetchProduk = () => {
    setLoading(true);
    adminApi
      .getProduk()
      .then((res) => {
        setProduk(res.data || []);
        setError("");
      })
      .catch((err) => {
        console.error("Error fetching produk:", err);
        setError("Gagal memuat data produk");
      })
      .finally(() => setLoading(false));
  };

  const handleDelete = async (id) => {
    if (!window.confirm("Yakin ingin menghapus produk ini?")) return;
    try {
      await adminApi.deleteProduk(id);
      setProduk(produk.filter((item) => item.id_produk !== id));
      alert("Produk berhasil dihapus");
    } catch (error) {
      console.error("Error deleting produk:", error);
      alert(error.body?.message || "Gagal menghapus produk");
    }
  };

  if (loading) {
    return (
      <div className="container-fluid">
        <div className="row">
          <div className="col-12 text-center py-5">
            <div className="spinner-border text-primary" role="status">
              <span className="visually-hidden">Loading...</span>
            </div>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="container-fluid">
      <div className="row">
        <div className="col-12">
          {error && (
            <div className="alert alert-danger alert-dismissible fade show mb-3" role="alert">
              {error}
              <button type="button" className="btn-close" onClick={() => setError("")}></button>
            </div>
          )}

          <div className="d-flex justify-content-between align-items-center mb-4">
            <div>
              <h5 className="fw-bold mb-0">Manajemen Produk</h5>
              <small className="text-secondary">{produk.length} produk</small>
            </div>
            <Link to="/admin/produk/tambah" className="btn btn-dark btn-sm">
              + Tambah produk
            </Link>
          </div>

          <div className="bg-white rounded shadow-sm overflow-hidden">
            <div className="table-responsive">
              <table className="table table-sm table-hover mb-0 align-middle">
                <thead className="bg-light">
                  <tr>
                    <th style={{ width: 60 }}>Gambar</th>
                    <th>Nama</th>
                    <th>Kategori</th>
                    <th className="text-end">Harga</th>
                    <th className="text-end" style={{ width: 150 }}>Tindakan</th>
                  </tr>
                </thead>
                <tbody>
                  {produk.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="text-center text-secondary py-4">
                        Belum ada produk. Klik "Tambah produk" untuk menambah.
                      </td>
                    </tr>
                  ) : (
                    produk.map((item) => (
                      <tr key={item.id_produk}>
                        <td>
                          <img
                            src={mediaUrl(item.gambar)}
                            alt={item.nama_produk}
                            className="rounded"
                            style={{ width: 44, height: 44, objectFit: "cover", background: "#f5f5f5" }}
                            onError={onImgError}
                          />
                        </td>
                        <td>
                          <span className="fw-medium">{item.nama_produk}</span>
                        </td>
                        <td className="text-secondary small">{item.kategori || "-"}</td>
                        <td className="text-end fw-bold">{formatRupiah(item.harga)}</td>
                        <td className="text-end">
                          <div className="d-flex gap-1 justify-content-end">
                            {/* Detail - Icon mata */}
                            <Link
                              to={`/admin/produk/${item.id_produk}`}
                              className="btn btn-outline-secondary btn-sm"
                              title="Detail"
                              style={{ width: "32px", height: "32px", padding: "0", display: "flex", alignItems: "center", justifyContent: "center" }}
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                <path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8zM1.173 8a13.133 13.133 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5c2.12 0 3.879 1.168 5.168 2.457A13.133 13.133 0 0 1 14.828 8c-.058.087-.122.183-.195.288-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5c-2.12 0-3.879-1.168-5.168-2.457A13.134 13.134 0 0 1 1.172 8z"/>
                                <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5zM4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0z"/>
                              </svg>
                            </Link>

                            {/* Ubah - Icon pensil */}
                            <Link
                              to={`/admin/produk/edit/${item.id_produk}`}
                              className="btn btn-outline-primary btn-sm"
                              title="Ubah"
                              style={{ width: "32px", height: "32px", padding: "0", display: "flex", alignItems: "center", justifyContent: "center" }}
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/>
                              </svg>
                            </Link>

                            {/* Hapus - Icon tong sampah */}
                            <button
                              onClick={() => handleDelete(item.id_produk)}
                              className="btn btn-outline-danger btn-sm"
                              title="Hapus"
                              style={{ width: "32px", height: "32px", padding: "0", display: "flex", alignItems: "center", justifyContent: "center" }}
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z"/>
                                <path fillRule="evenodd" d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z"/>
                              </svg>
                            </button>
                          </div>
                        </td>
                      </tr>
                    ))
                  )}
                </tbody>
              </table>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}