...
export default function AdminArtikelPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert(error.body?.message || "Gagal menghapus artikel");
    }
  };
      ...
      <div className="row">
        <div className="col-12">
          ...
          <div className="d-flex justify-content-between align-items-center mb-4">
            <div>
              <h5 className="fw-bold mb-0">Manajemen Artikel</h5>
              <small className="text-secondary">{artikel.length} artikel</small>
            </div>
            <Link to="/admin/artikel/tambah" className="btn btn-dark btn-sm">
              + Artikel baru
            </Link>
          </div>
                      ...
                      <tr key={item.id}>
                        <td>
                          <img
                            ...
                            alt={item.judul}
                            className="rounded"
                            style={{ width: 50, height: 50, objectFit: "cover", background: "#f5f5f5" }}
                            onError={onImgError}
                          />
                        </td>
                        <td>
                          <span className="fw-medium">{item.judul}</span>
                          <br />
                          <small className="text-secondary">{item.ringkasan?.slice(0, 80)}...</small>
                        </td>
                        <td className="text-secondary small">{formatTanggal(item.created_at)}</td>
                        <td className="text-end">
                          <div className="d-flex gap-2 justify-content-end">
                            {/* Detail - icon mata/lihat */}
                            <Link
